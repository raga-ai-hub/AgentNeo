    ...
    const loadTimelineData = async () => {
      if (selectedTraceId) {
        try {
          ...
          const convertedData: TimelineData[] =
            convertTraceToTimelineData(traceData)
  ...
  const convertTraceToTimelineData = (traceData: any): TimelineData[] => {
    ...
    timelineEvents.push({
      ...
      duration:
        (new Date(traceData.end_time).getTime() -
          new Date(traceData.start_time).getTime()) /
        1000,
      color: '#4285F4',
      type: 'Trace',
      row: 'Main',
    ...
    traceData.llm_calls?.forEach((llm: any) => {
      timelineEvents.push({
        ...
        duration:
          (new Date(llm.end_time).getTime() -
            new Date(llm.start_time).getTime()) /
          1000,
        color: '#34A853',
        type: 'LLM',
        row: 'LLM',
    ...
    traceData.tool_calls?.forEach((tool: any) => {
      const duration =
        (new Date(tool.end_time).getTime() -
          new Date(tool.start_time).getTime()) /
        1000
      timelineEvents.push({
        name: tool.name,
    ...
    traceData.agent_calls?.forEach((agent: any) => {
      ...
      timelineEvents.push({
        ...
        duration:
          (new Date(agent.end_time).getTime() -
            new Date(agent.start_time).getTime()) /
          1000,
        color: '#4285F4',
        type: 'Agent',
        row: 'Agent',
    ...
    ...(timelineData.map(
      event =>
        (new Date(event.endTime).getTime() -
          new Date(timelineData[0].startTime).getTime()) /
        1000
    ) || [0])
  )
  ...
  useEffect(() => {
    if (timelineRef.current) {
      ...
      const scrollPosition =
        (currentTime / totalDuration) * timelineWidth * (zoom / 100) -
        timelineWidth / 2
        ...
        <div className='flex'>
          ...
          <div className='w-1/4 pl-4 border-l border-gray-200'>
            <TimelineDetails
              ...
              overlappingEvents={
                selectedEvent ? getOverlappingEvents(selectedEvent) : []
              }
              counts={
